...
(en este caso, hemos seguido las recomendaciones de la práctica y se han
escogido las ciudades situadas más al norte, este y oeste, respectivamente). 
Posteriormente,
...
\begin{algorithm}
    ...
    \KwData{$n \geq 3$}
    $S = []$\;
    $S \gets c_\alpha $, con $\alpha \in \{j \in \mathbb N : c_j.y \geq c_i.y, \forall i \in \mathbb \{1,2,\cdots,n\}\}$ \Comment*[r]{Ciudad más al norte}
    $S \gets c_\beta $, con $\beta \in \{j \in \mathbb N : c_j.x \geq c_i.x, \forall i \in \mathbb \{1,2,\cdots,n\}\}$ \Comment*[r]{Ciudad más al este}
    \While{$N \neq 0$}{
      \eIf{$N$ is even}{
        $X \gets X \times X$\;
        $N \gets \frac{N}{2}$ \Comment*[r]{This is a comment}
