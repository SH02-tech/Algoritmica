...
# GRUPO 8
g++ -D REPETITIONS=1 -D SIZE_VECTOR=1 src/ejercicio-1-comp-fija-no-repetidos.cpp -o ejercicio-1-comp-fija-no-repetidos
...
./MainGraph.sh --xlabel "Número de componentes del vector" --ylabel "Tiempo (en nanosegundos)" --min-x 25000 --max-x 10000000 --num-points 5  --repetitions-per-point $REPETICIONES ejercicio-1-comp-fija-no-repetidos
rm ejercicio